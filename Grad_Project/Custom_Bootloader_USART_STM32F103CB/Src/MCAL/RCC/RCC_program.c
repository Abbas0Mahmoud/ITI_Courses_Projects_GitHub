...
void MRCC_voidInit(void)
{
    ...
    /*      HSI Clock Source is Selected        */
        ...
        {
            /*      Assign System Clock Frequency       */
            RCC_Clks.SysClk_F = HSI_CLK_F;
            /*      Check Buses Frequency       */
            MRCC_voidCheckBusesClockFrequency(AHB_PRESCALER,APB1_PRESCALER,APB2_PRESCALER);
    ...
    /*      PLL Clock Source is Selected        */
        ...
        /*      HSI Clock Divided by 2 Is Selected as PLL Input Clock Source        */
            ...
            if(Local_u8ClkStatus == CLK_READY)
            {
                ...
                /*      Disable PLL Clock Source        */
                MRCC_voidSetClkSourceStatus(RCC_CFGR_SW_PLL_SYSCLK,CLK_SRC_OFF);
                /*      Calculating System Clock Frequency      */
                RCC_Clks.SysClk_F = ((HSI_CLK_F/2)*(PLL_MULTIPLICATION_FACTOR+2));
                /*      Checking if PLL Configurations are Within constrains        */
                if(( RCC_Clks.SysClk_F <= SYS_CLK_F_MAX))
                {
        ...
        /*      HSE Is Selected as PLL Input Clock Source      */
            ...
            if(Local_u8ClkStatus == CLK_READY)
            {
                ...
                MRCC_voidSetClkSourceStatus(RCC_CFGR_SW_PLL_SYSCLK,CLK_SRC_OFF);
                /*      Calculating System Clock Frequency      */
                RCC_Clks.SysClk_F = ((HSE_CLK_F)*(PLL_MULTIPLICATION_FACTOR+2));
                /*      Checking if PLL Configurations are Within constrains        */
                if((RCC_Clks.SysClk_F <= SYS_CLK_F_MAX))
                {
                    /*      Setting PLL Source and Multiplication Factor        */
        ...
        /*      HSE Divided By 2 Is Selected as PLL Input Clock Source      */
            ...
            if(Local_u8ClkStatus == CLK_READY)
            {
                ...
                MRCC_voidSetClkSourceStatus(RCC_CFGR_SW_PLL_SYSCLK,CLK_SRC_OFF);
                /*      Calculating System Clock Frequency      */
                RCC_Clks.SysClk_F = ((HSE_CLK_F/2)*(PLL_MULTIPLICATION_FACTOR+2));
                /*      Checking if PLL Configurations are Within constrains        */
                if((RCC_Clks.SysClk_F <= SYS_CLK_F_MAX))
                {
                    /*      Setting PLL Source and Multiplication Factor        */
...
void MRCC_voidSetBusesPrescalers(u8 Copy_u8AHBPrescaler,u8 Copy_u8APB1Prescaler,u8 Copy_u8APB2Prescaler)
{
    ...
    RCC->CFGR |=    ((Copy_u8AHBPrescaler << RCC_CFGR_HPRE_BIT0)|
                    ...
                    (Copy_u8APB2Prescaler << RCC_CFGR_PPRE2_BIT0));
}
/*____________________________________________________________________________________________________________*/
...
void MRCC_voidResetPeripheral(u8 Copy_u8BusName,u8 Copy_u8Peripheral)
{
    ...
    switch(Copy_u8BusName)
    {
        ...
        case APB1_BUS:
            /*      Enable Peripheral Clock Bit in APB2ENR Register      */
            SET_BIT(RCC->APB1RSTR,Copy_u8Peripheral);
        break;
        /*      Peripheral is Connected to APB2 Bus      */
        case APB2_BUS:
            /*      Enable Peripheral Clock Bit in APB2ENR Register      */
            SET_BIT(RCC->APB2RSTR,Copy_u8Peripheral);
        break;
    }
}
...
*/
/* ----------------------------------------------------------------------------------------------------------- */
void MRCC_voidCheckBusesClockFrequency(u8 Copy_u8AHBPrescaler,u8 Copy_u8APB1Prescaler,u8 Copy_u8APB2Prescaler)
{
	switch(Copy_u8AHBPrescaler)
	{
		case RCC_CFGR_HPRE_SYSCLK_DIV_1   : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 1);   break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_2   : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 2);   break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_4   : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 4);   break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_8   : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 8);   break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_16  : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 16);  break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_64  : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 64);  break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_128 : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 128); break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_256 : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 256); break;
		case RCC_CFGR_HPRE_SYSCLK_DIV_512 : RCC_Clks.AHBClk_F = (RCC_Clks.SysClk_F / 512); break;
	}
	if(RCC_Clks.APB1Clk_F > AHB_CLK_MAX_F)
	{
		/*	Error	*/
	}
	switch(Copy_u8APB1Prescaler)
	{
		case RCC_CFGR_PPRE1_HCLK_DIV_1  : RCC_Clks.APB1Clk_F = (RCC_Clks.AHBClk_F / 1);  break;
		case RCC_CFGR_PPRE1_HCLK_DIV_2  : RCC_Clks.APB1Clk_F = (RCC_Clks.AHBClk_F / 2);  break;
		case RCC_CFGR_PPRE1_HCLK_DIV_4  : RCC_Clks.APB1Clk_F = (RCC_Clks.AHBClk_F / 4);  break;
		case RCC_CFGR_PPRE1_HCLK_DIV_8  : RCC_Clks.APB1Clk_F = (RCC_Clks.AHBClk_F / 8);  break;
		case RCC_CFGR_PPRE1_HCLK_DIV_16 : RCC_Clks.APB1Clk_F = (RCC_Clks.AHBClk_F / 16); break;
	}
	if(RCC_Clks.APB1Clk_F > APB1_CLK_MAX_F)
	{
		/*	Error	*/
	}
	switch(Copy_u8APB2Prescaler)
	{
		case RCC_CFGR_PPRE2_HCLK_DIV_1  : RCC_Clks.APB2Clk_F = (RCC_Clks.AHBClk_F / 1);  break;
		case RCC_CFGR_PPRE2_HCLK_DIV_2  : RCC_Clks.APB2Clk_F = (RCC_Clks.AHBClk_F / 2);  break;
		case RCC_CFGR_PPRE2_HCLK_DIV_4  : RCC_Clks.APB2Clk_F = (RCC_Clks.AHBClk_F / 4);  break;
		case RCC_CFGR_PPRE2_HCLK_DIV_8  : RCC_Clks.APB2Clk_F = (RCC_Clks.AHBClk_F / 8);  break;
		case RCC_CFGR_PPRE2_HCLK_DIV_16 : RCC_Clks.APB2Clk_F = (RCC_Clks.AHBClk_F / 16); break;
	}
	if(RCC_Clks.APB2Clk_F > APB2_CLK_MAX_F)
	{
		/*	Error	*/
	}
}
